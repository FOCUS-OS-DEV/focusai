...
export default async function BlogPage({ searchParams }: PageProps) {
  ...
  // Fetch shared content and page-specific content
  const { pages, contact } = await getSharedContent()
  const blogContent = pages?.blog
      ...
      <section className="relative py-20 lg:py-28 overflow-hidden" style={{ background: 'linear-gradient(180deg, #f3e8ff 0%, #fce7f3 100%)' }}>
        ...
        <div className="container mx-auto px-4 max-w-6xl relative z-10 text-center">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-6">
            {blogContent?.hero?.title || 'הבלוג'}{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a855f7] to-[#ec4899]">
              {blogContent?.hero?.titleHighlight || 'שלנו'}
            </span>
          </h1>
          <p className="text-lg md:text-xl text-gray-600 max-w-2xl mx-auto">
            {blogContent?.hero?.subtitle || 'מאמרים, טיפים ותובנות מעולם הבינה המלאכותית והאוטומציה העסקית'}
          </p>
        </div>
      </section>
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              {params.search || params.category
                ? (blogContent?.emptyState?.noResults || 'לא נמצאו מאמרים התואמים את החיפוש')
                : (blogContent?.emptyState?.noPosts || 'עוד אין מאמרים בבלוג')}
            </h2>
            <p className="text-gray-600 mb-8">
              {params.search || params.category
                ? (blogContent?.emptyState?.filterSubtitle || 'נסו לשנות את הפילטרים או לחפש משהו אחר')
                : (blogContent?.emptyState?.noPostsSubtitle || 'מאמרים חדשים יתווספו בקרוב!')}
            </p>
            {(params.search || params.category) && (
              <Link
                href="/blog"
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-purple-100 shadow-[0_20px_50px_rgba(168,85,247,0.15)] text-center">
            ...
            <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-4">
              {blogContent?.cta?.title || 'רוצים ללמוד עוד?'}
            </h2>
            <p className="text-lg text-gray-600 mb-8 max-w-xl mx-auto">
              {blogContent?.cta?.subtitle || 'הצטרפו לקורסים שלנו ולמדו AI ואוטומציה בצורה מעשית ומקצועית'}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href={blogContent?.cta?.primaryButton?.link || '/courses'}
              ...
              >
                <span>{blogContent?.cta?.primaryButton?.text || 'הקורסים שלנו'}</span>
              ...
              >
                ...
                <span>{blogContent?.cta?.secondaryButton?.text || 'WhatsApp'}</span>
              </a>
            </div>
          </div>
        </div>
