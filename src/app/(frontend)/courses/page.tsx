...
export default async function CoursesPage({ searchParams }: PageProps) {
  ...
  const coursesContent = pages?.courses
      ...
      <section className="relative py-20 lg:py-28 overflow-hidden" style={{ background: 'linear-gradient(180deg, #f3e8ff 0%, #fce7f3 100%)' }}>
        ...
        <div className="container mx-auto px-4 max-w-6xl relative z-10 text-center">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-6">
            {coursesContent?.hero?.title || 'המסלולים'}{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a855f7] to-[#ec4899]">
              {coursesContent?.hero?.titleHighlight || 'שלנו'}
            </span>
          </h1>
          <p className="text-lg md:text-xl text-gray-600 max-w-2xl mx-auto">
            {coursesContent?.hero?.subtitle || 'מגוון הכשרות מעשיות בתחום הבינה המלאכותית - מהיסודות ועד לרמה המתקדמת'}
          </p>
        </div>
      </section>
        ...
        {courses.docs.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              {coursesContent?.emptyState?.title || 'לא נמצאו קורסים התואמים את החיפוש'}
            </h2>
            <p className="text-gray-600 mb-8">
              {coursesContent?.emptyState?.subtitle || 'נסו לשנות את הפילטרים או לחפש משהו אחר'}
            </p>
            <Link
              href="/courses"
            ...
            >
              {coursesContent?.emptyState?.buttonText || 'הצג את כל הקורסים'}
            </Link>
          </div>
        ) : (
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-purple-100 shadow-[0_20px_50px_rgba(168,85,247,0.15)] text-center">
            ...
            <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-4">
              {coursesContent?.cta?.title || 'לא בטוחים מה מתאים לכם?'}
            </h2>
            <p className="text-lg text-gray-600 mb-8 max-w-xl mx-auto">
              {coursesContent?.cta?.subtitle || 'נשמח לעזור לכם לבחור את המסלול המתאים ביותר לצרכים שלכם'}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                href={coursesContent?.cta?.primaryButton?.link || '#contact'}
              ...
              >
                ...
                <span>{coursesContent?.cta?.primaryButton?.text || 'שיחת ייעוץ חינם'}</span>
              </a>
              <a
              ...
              >
                ...
                <span>{coursesContent?.cta?.secondaryButton?.text || 'WhatsApp'}</span>
              </a>
            </div>
          </div>
        </div>
