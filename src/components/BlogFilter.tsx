...
export default function BlogFilter({ categories }: BlogFilterProps) {
  ...
  const hasActiveFilters = search || currentCategory !== 'all'
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {categories.map((category) => (
          <button
            ...
            style={currentCategory === String(category.id) ? { backgroundColor: category.color || '#a855f7' } : undefined}
          >
            {category.name}
          </button>
        ))}
          ...
          <div className="flex items-center gap-2 flex-wrap text-sm">
            ...
            {currentCategory !== 'all' && (
              <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-purple-100 text-purple-800">
                ...
                קטגוריה: {categories.find(c => String(c.id) === currentCategory)?.name}
              </span>
            )}
